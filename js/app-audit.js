    ...
    filtered.forEach(item => {
      ...
      tr.innerHTML = `
        ...
        <td class="py-2.5 px-3 text-right bg-slate-100/90 border-x border-slate-200">
          <input type="number" min="0" value="${item.qtdEmpenho}" ${!canEditValues ? 'disabled' : ''} onchange="app.audit.updateVal(${item.id}, 'qtdEmpenho', this.value)" class="table-num w-20 text-right px-2 py-1 text-xs border border-slate-300 rounded bg-white shadow-xs focus:border-slate-600 font-bold text-slate-800 ${!canEditValues ? 'opacity-75 cursor-not-allowed' : ''}">
        </td>
        <td class="py-2.5 px-3 text-right">
          <input type="number" min="0" value="${item.saldoAnterior}" ${!canEditValues ? 'disabled' : ''} onchange="app.audit.updateVal(${item.id}, 'saldoAnterior', this.value)" class="table-num w-20 text-right px-2 py-1 text-xs border border-slate-300 rounded bg-white shadow-xs focus:border-blue-500 font-semibold ${!canEditValues ? 'opacity-75 cursor-not-allowed' : ''}">
        </td>
        <td class="py-2.5 px-3 text-right bg-blue-50/70 border-x border-blue-100">
          <input type="number" min="0" value="${item.faturado}" ${!canEditValues ? 'disabled' : ''} onchange="app.audit.updateVal(${item.id}, 'faturado', this.value)" class="table-num w-20 text-right px-2 py-1 text-xs border border-blue-300 rounded bg-white shadow-xs focus:border-blue-600 font-bold text-blue-950 ${!canEditValues ? 'opacity-75 cursor-not-allowed' : ''}">
        </td>
