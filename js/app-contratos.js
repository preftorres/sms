...
app.contratos = {
  ...
  openNewModal() {
    if (!app.permissions.can('panel_create')) {
      return alert("Seu perfil de acesso não tem permissão para cadastrar contratos no mural.");
    }
  ...
  openEditModal(id) {
    if (!app.permissions.can('panel_edit')) {
      return alert("Seu perfil de acesso não tem permissão para editar contratos.");
    }
  ...
  async toggleArchive(id) {
    if (!app.permissions.can('panel_archive')) {
      return alert("Seu perfil de acesso não tem permissão para arquivar contratos.");
    }
...
app.render.contratosHub = function(el) {
  ...
  const canCreate = app.permissions.can('panel_create');
  const canEdit = app.permissions.can('panel_edit');
  const canArchive = app.permissions.can('panel_archive');
    ...
    <div class="container mx-auto px-4 sm:px-6 py-6 sm:py-8 fade-in">
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-6 flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4">
            ...
            <div class="flex items-center gap-1 bg-slate-100 p-1 rounded-xl shrink-0">
                <button onclick="app.contratos.switchViewMode('CARDS')" title="Mural de Cards" class="p-1.5 rounded-lg text-xs font-bold transition ${isCardsMode ? 'bg-white text-blue-600 shadow-xs' : 'text-slate-500'}">
                    🗂️ Mural
                </button>
                <button onclick="app.contratos.switchViewMode('TABELA')" title="Tabela Analítica" class="p-1.5 rounded-lg text-xs font-bold transition ${!isCardsMode ? 'bg-white text-blue-600 shadow-xs' : 'text-slate-500'}">
                    📊 Tabela
                </button>
            </div>
        </div>
                              ...
                              return `
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="p-3.5 text-center whitespace-nowrap print:hidden">
                                        ${canEdit ? `<button onclick="app.contratos.openEditModal(${c.id})" class="text-slate-600 hover:text-amber-700 font-bold mr-1.5">✏️</button>` : ''}
                                        ${canArchive ? `<button onclick="app.contratos.toggleArchive(${c.id})" class="text-slate-400 hover:text-slate-700 font-bold mr-1.5" title="Arquivar">📁</button>` : ''}
