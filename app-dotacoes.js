 ...
 * MÓDULO DE DOTAÇÕES: Livro Digital, Exclusão Blindada e Rastreabilidade
...
app.dotacoes = {
  ...
  // =========================================================================
  // EXCLUSÃO BLINDADA (LIXEIRA, BLOQUEIO DE COLA E VALIDAÇÃO DE DIGITAÇÃO)
  // =========================================================================
  openDeleteModal(id) {
    if (!app.admin.canEditDotacao()) {
      return alert("Apenas administradores e gestores financeiros têm permissão para excluir solicitações.");
    }
    ...
    // Preenche o resumo visual dos dados que serão destruídos
  ...
  // BLOQUEIA A COLAGEM (Ctrl + V)
  blockPaste(e) {
    e.preventDefault();
    alert("Por segurança institucional, não é permitido colar o número. Digite os 5 dígitos manualmente para autorizar a exclusão.");
    return false;
  },

  // VALIDA A DIGITAÇÃO EXATA DO PROCESSO
  async confirmDelete(e) {
    ...
    if (typed !== String(target.processo).trim()) {
      errorEl.textContent = `O número digitado (${typed}) não confere com o processo (${target.processo}). Digitação incorreta!`;
      errorEl.classList.remove('hidden');
      document.getElementById('delete-dot-typed-processo').focus();
      return;
    }

    // Se bateu exatamente o número do processo, efetua a exclusão
    ...
    alert(`Processo nº ${target.processo} foi excluído definitivamente do livro e da planilha!`);
  },
  ...
  // RENDERIZAÇÃO DA TABELA (COM LÁPIS E LIXEIRA LADO A LADO)
  renderTableRows() {
        ...
        <tr class="border-b border-slate-200 transition ${isPend ? 'bg-amber-50/40 hover:bg-amber-100/40' : 'bg-white hover:bg-slate-50'}">
          ...
          <td class="p-3.5 text-center whitespace-nowrap">
            <div class="flex items-center justify-center gap-1.5">
              ...
              <!-- BOTÃO DE EXCLUSÃO BLINDADA (LIXEIRA) -->
              ${canEdit ? `
